...
def load_yaml(conn, versions):
    ...
    '''
    user_id = users.get_user_id(conn, versions['user'])
    for section in versions['versions']:
        if 'add' in section:
            load_add_versions(conn, user_id, section['add'])
        elif 'change' in section:
            load_change_versions(conn, user_id, section['change'])
...
def load_add_versions(conn, user_id, versions):
...
def load_change_versions(conn, user_id, changes):
